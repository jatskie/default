 ...
 * @return      void
*/
class thememount_custom_menus_walker extends Walker_Nav_Menu
{
      function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
      {
           ...
           $class_names = $value = '';
			...
			if($depth == 0){
				$prepend = '<span class="thememount-menu-text">';
				$append  = '</span>';
			} else {
				$prepend = '<span>';
				$append  = '</span>';
           ...
           if($depth != 0)
           {
            ...
            if( $depth == 0 ){
				// Icon
				$item_output .= !empty( $item->icon ) ? '<span class="gm-menu-icon menu-with-icon"><i class="tmicon-'  . esc_attr( $item->icon       ) .'"></i></span>' : '<span class="gm-menu-icon menu-without-icon"></span>'; // Adding Icon Class to A tag
				// Image
