...
function thememount_one_click_js_code() {
	$images   = array();
	$images[] = get_template_directory_uri() . '/inc/redux-framework/ReduxCore/inc/fields/thememount_one_click_demo_content/import-alert.jpg';
	$images[] = get_template_directory_uri() . '/inc/redux-framework/ReduxCore/inc/fields/thememount_one_click_demo_content/import-loader.gif';
	$images[] = get_template_directory_uri() . '/inc/redux-framework/ReduxCore/inc/fields/thememount_one_click_demo_content/import-success.jpg';
	...
	jQuery(document).ready(function($) {
		...
		jQuery('#thememount_one_click_demo_content_option').click(function() {
			jQuery('#import-demo-data-results-wrapper').slideDown();
			$(this).attr('disabled', 'disabled');
		});
		
		jQuery('#thememount_one_click_demo_content_cancel').click(function() {
			jQuery('#import-demo-data-results-wrapper').slideUp(function(){
				$('#thememount_one_click_demo_content_option').prop("disabled", false);
			});
		...
		jQuery('#thememount_one_click_demo_content').click(function() {
			...
			resultDiv.addClass('thememount-import-demo-progress'); /* Adding loader class */
			...
			$.ajax({
				...
				dataType: "json",
				data: {
					'action'    : 'howes_install_demo_data',
					'color'     : color,
					'subaction' : 'start'
				},
				beforeSend: function() {
					//resultDiv.html('<p id="install-demo-data-started">' + howesVars.strInstallingDemoData + '</p>').show().removeClass('error');
					resultDiv.html('<p id="install-demo-data-started">Starting <strong>'+color+'</strong> Demo Content Setup</p>').show().removeClass('error');
				},
						...
						if( result != null && typeof( result ) == 'object' ) {
							...
							if( result.answer == 'ok' ) {
								...
								$.ajax({
									...
									error: function(request, status, error) {
										//resultDiv.html( '<p><strong style="color: red">' + howesVars.strError + ": " + request.status + '</p>' );
							...
							if( result.answer == 'finished' ) {
								...
								if( color == 'dark' ){
									...
									resultDiv.append('<p><strong>All finished... Enjoy :) </strong></p><div class="tm-import-done">The next step is to insert Slider and some small changes. <a href="http://howes.thememount.com/documentation/democontent.html" target="_blank">Click here for next steps</a>.</div>');
									resultDiv.addClass('thememount-import-demo-success');
								}
							...
							resultDiv.append( '<p><strong style="color: red">' + howesVars.strError + ":</strong> " + howesVars.strWrongServerAnswer + '</p>' ).addClass('error');
							button.removeAttr('disabled');
							$('#install-demo-data-started').remove();
...
if( !class_exists( 'thememount_one_click_demo_setup' ) ) {
	...
	class thememount_one_click_demo_setup{
		
		
		function __construct(){
			add_action( 'wp_ajax_howes_install_demo_data', array( &$this , 'ajax_install_demo_data' ) );
			/*include_once('demo-content-scripts.php');
			global $thememount_demo_installation;
			$thememount_demo_installation = new thememount_demo_installation;*/
		}
		...
		function ajax_install_demo_data() {
			/*global $thememount_demo_content;
			include_once('wordpress-importer/parsers.php');
			$WXR_Parser         = new WXR_Parser;
			$thememount_demo_content = $WXR_Parser->parse( dirname( __FILE__ ).'/demo.xml' );*/
			
			/*if( !is_array($thememount_demo_content) ){
				$thememount_demo_content = $this->setup_demo_content_array();
			}*/
			...
			if( isset($_POST['color']) && $_POST['color']=='dark' ){
				$filename = 'demo-dark.xml';
			} else {
				$filename = 'demo.xml';
			}
			$WP_Import->import_start( dirname( __FILE__ ).'/'.$filename );
			...
			switch( $subaction ) {
				...
				case( 'start' ):
					...
					$answer['message']        = __('Inserting Categories...', 'howes');
					$answer['data']           = '';
				...
				case( 'install_demo_cat' ):
					...
					$answer['message']        = __('All Categories were inserted successfully. Inserting Tags...', 'howes');
				...
				case( 'install_demo_tags' ):
					...
					$answer['message']        = __('All Tags were inserted successfully. Inserting Terms...', 'howes');
					$answer['data']           = base64_encode( serialize( $data ) );
				...
				case( 'install_demo_terms' ):
					...
					$answer['message']        = __('All Terms were inserted successfully. Inserting Posts...', 'howes');
					$answer['data']           = base64_encode( serialize( $data ) );
				...
				case( 'install_demo_posts' ):
					...
					$answer['message']        = __('All Posts were inserted successfully. Importing images...', 'howes');
					$answer['data']           = base64_encode( serialize( $data ) );
				...
				case( 'install_demo_images' ):
					...
					$answer['message']        = __('All Images were inserted successfully. Setting the widgets and options...', 'howes');
					$answer['data']           = base64_encode( serialize( $data ) );
				...
				case( 'install_demo_settings' ):
					...
					$content = file_get_contents( dirname( __FILE__ ).'/demo.xml' );
...
// For AJAX callback
$thememount_one_click_demo_setup = new thememount_one_click_demo_setup;
