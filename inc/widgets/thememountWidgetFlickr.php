...
add_action( 'widgets_init', 'thememount_widget_flickr' );

function thememount_widget_flickr() {
	register_widget( 'thememount_widget_flickr' );
}

class thememount_widget_flickr extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'thememount_widget_flickr',
							  'description' => __('Show photos from Flickr.','howes') );
		...
		parent::__construct(
			...
			__('ThemeMount Flickr Widget', 'howes'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		echo '<div class="thememount_widget_flickr_wrapper">'; ?>
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php echo __('Title', 'howes'); ?>:</label>
