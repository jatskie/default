<?php
// [clients]
if( !function_exists('thememount_sc_clients') ){
function thememount_sc_clients($atts, $content=NULL ) {
	extract( shortcode_atts( array(
		'title'    => '',
		'subtitle' => '',
		'align'    => 'left',
		//'sepicon'  => 'NO_ICON',
		'show'     => '4',
		'column'   => 'four',
		'group'    => '',
		'view'     => 'default',
	), $atts ) );
	
	$return = '';
	$rowClass = ( $view != 'carousel' ) ? 'row' : '' ;
	$rowClass = ( $view == 'carousel' ) ? $rowClass : $rowClass.' multi-columns-row' ;

	
	
	switch($column){
		case 'two':
			$boxClass = 'col-lg-6 col-sm-6 col-md-6 col-xs-12';
			break;
		case 'three':
		default:
			$boxClass = 'col-lg-4 col-sm-6 col-md-4 col-xs-12';
			break;
		case 'four':
			$boxClass = 'col-lg-3 col-sm-6 col-md-3 col-xs-12';
			break;
	}
	if( $view=='carousel' ){ $boxClass = 'thememount-full'; }
	
	$args = array(
		'post_type'      => 'client',
		'posts_per_page' => $show,
	);
	
	
	// Group
	if( $group!='' ){
		$return .= '<!-- Group Active -->';
		$args['tax_query'] = array(
								array(
									'taxonomy' => 'client_group',
									'field' => 'slug',
									'terms' => $group
								),
							);
	}
	
	$results = new WP_Query( $args );
	
	//var_dump($results);
	
	// The Loop
	if ( $results->have_posts() ) {
		$return .= "\n\t".'<div class="thememount-client-wrapper container thememount-client-view-'.$view.' thememount-client-column-'.$column.'">';
		if( trim($title)!='' ){
			$return .= "\n\t" . do_shortcode('[heading text="'.$title.'" subtext="'.do_shortcode($subtitle).'" tag="h2" style="linedot" align="'.$align.'"]');
		}
		
		$return .= '<div class="'.$rowClass.' thememount-clients">';
		while ( $results->have_posts() ) {
			$results->the_post();
			$link = trim( get_post_meta( get_the_ID(), '_thememount_clients_details_clienturl', true ) );
			$link = ( $link!='' ) ? $link : '#' ;
			
			if( has_post_thumbnail() ){
				$return .= '<div class="'.$boxClass.'">';
				$return .= '<a href="'.$link.'" target="_blank" title="'.get_the_title().'">';
				$return .= get_the_post_thumbnail( get_the_ID(), 'full' );
				$return .= '</a>';
				$return .= '</div>';
			} else {
				$return .= '<!-- No Featured Image For this Client -->';
			}
		}
		$return .= "\n\t".'</div></div>';
		
	} else {
		// no posts found
	}
	/* Restore original Post Data */
	wp_reset_postdata();
	
	return $return;
}
}
add_shortcode( 'clients', 'thememount_sc_clients' );